...
uint8_t LaunchPad_Input(void){
  ...
  uint8_t p1p1 = ~ GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN1) & 0b00000001;
  uint8_t p1p4 = ~ GPIO_getInputPinValue(GPIO_PORT_P1, GPIO_PIN4) & 0b00000001;

  // use bit-shifting to return appropriate output
  uint8_t answer = (p1p1) | (p1p4 << 1);
  return answer;
}
